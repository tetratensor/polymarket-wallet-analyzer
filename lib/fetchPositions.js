...
/**
 * Polymarket's official volume metric for the wallet, when a leaderboard
 * window matches the requested days. Note this metric is NOT cash notional:
 * book trades count at USDC paid, but matched buy-buy fills (complete-set
 * mints, common in cheap two-sided markets) are credited at ~$1 per share —
 * verified to the cent against raw fills. Returns null when unavailable.
 */
export async function fetchOfficialVolume(wallet, { days, fetchImpl = fetch }) {
  const window = { 1: '1d', 7: '7d', 30: '30d' }[days];
  if (!window) return null;
  try {
    const data = await getJson(`${LB_API}/volume?window=${window}&limit=1&address=${wallet}`, fetchImpl);
    if (Array.isArray(data) && data[0] && typeof data[0].amount === 'number') {
      return { window, amount: data[0].amount };
    }
  } catch {
    // leaderboard API is best-effort garnish; the dashboard works without it
  }
  return null;
}
